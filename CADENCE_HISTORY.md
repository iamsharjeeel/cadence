...
- **`created_by_timer`** column on `time_entries` (`20260630000002_time_entries_created_by_timer.sql`; applied on prod, not backfilled).
- `saveTimerEntries` sets `created_by_timer: true` on timer stop.
- `/app/time-tracked` filters `created_by_timer = true` (replaces incorrect `entry_mode = time_range` proxy).
- Timer vs manual hour summary stats on Time Tracked (respects window + project filters).
- `TimerProvenanceMarker` on weekly log rows and timesheet detail for timer-created entries.
- Dashboard "hours by period" stacked breakdown skipped — single-series `HoursLineChart` would need a new stacked chart component and per-period source split in `getEmployeeDashboard`.
