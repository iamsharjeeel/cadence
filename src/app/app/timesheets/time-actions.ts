...
import {
  ...
  type TimeTrackingDataResult,
} from "@/lib/time/get-time-tracking-data";
import { mondayOfWeek, toIsoDate, weekPeriodFromMonday } from "@/lib/time/periods";
import {
  canSubmitWeek,
  computeWeekStats,
  overtimeHours,
...
export async function ensureTimesheetForWeek(
  ...
  return ensureTimesheetForWeekForProfile(profile, weekMonday);
}

/** Full week-load payload for the log view. */
export async function getTimeTrackingData(weekMonday: string): Promise<TimeTrackingDataResult> {
  const profile = await requireActiveProfile();
  if (!profile.org_id) return { ok: false, message: "Your account has no organization." };
