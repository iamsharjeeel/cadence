...
export default async function TimesheetsPage({
  ...
  // All users see their own personal timesheet list.
  // Managers also have access to team view via the "Team" tab / filters.
  const showTeamView = isManager && searchParams.view !== "personal";

  const statusFilter = (searchParams.status ?? "") as TimesheetStatus | "";
  const employeeFilter = showTeamView ? (searchParams.employee ?? "") : "";
  const fromFilter = searchParams.from ?? "";
  const toFilter = searchParams.to ?? "";
  const orgFilter = isSuperadmin && showTeamView ? searchParams.org ?? "" : "";
  ...
  if (!showTeamView) {
    ...
    if (!isSuperadmin && ctx.activeOrgId) query = query.eq("org_id", ctx.activeOrgId);
  ...
  if (sort === "total") {
    query = query.order("calculated_total", { ascending: dir === "asc", nullsFirst: false });
  ...
  const pageDescription = showTeamView
    ? "Review employee timesheets — including live drafts in progress."
    : "Your timesheet history. Log time and submit for approval.";
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <TimesheetPageActions showExport={isManager} showLogTime />
        }
      />
      ...
      <Card>
        <CardHeader>
          <CardTitle>{showTeamView ? "All timesheets" : "Your timesheets"}</CardTitle>
          <CardDescription>
            {timesheets.length}{" "}
            {timesheets.length === 1 ? "timesheet" : "timesheets"}
          </CardDescription>
          ...
          {timesheets.length === 0 ? (
            <div className="px-6 py-10">
              <EmptyState
                ...
                description={
                  showTeamView
                    ? "Submitted and draft timesheets will appear here."
                    : "Log your first timesheet using the button above."
                }
                action={
                  !showTeamView ? (
                    <Link href="/app/timesheets/log">
