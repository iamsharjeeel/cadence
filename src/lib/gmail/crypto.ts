...
function key(): Buffer {
  ...
  cachedKey = scryptSync(secret, "cadence-gmail-v1", 32);
  return cachedKey;
}

export function encryptGmailToken(plain: string): string {
...
export function decryptGmailToken(payload: string | null | undefined): string {
  if (!payload) return "";
