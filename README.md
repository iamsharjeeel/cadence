...
- **Personal role resolution.** In personal workspace (`activeOrgId = null`, non-superadmin), the effective app role resolves to an **admin-equivalent** context (shown as **Admin** on Profile) so solo users are not treated as org employees. Org workspace role resolution remains membership-driven (`owner/admin/employee`) and unchanged.
